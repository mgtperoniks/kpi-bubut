...
@section('content')
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('tracking.operator.pdf', $date) }}" class="inline-flex items-center justify-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
            ...
            <a href="{{ url('/export/operator/' . $date) }}" class="inline-flex items-center justify-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($rows as $row)
                        <tr class="odd:bg-white even:bg-gray-50 hover:bg-blue-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4">
                                @if(isset($shifts[$row->operator_code]))
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800 border border-gray-200">
                                        Shift {{ $shifts[$row->operator_code] }}
                                    </span>
                                @else
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-10 text-center text-gray-500 italic bg-gray-50">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p>Data KPI tidak ditemukan untuk tanggal ini</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
